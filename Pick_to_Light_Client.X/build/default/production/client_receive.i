...
# 1 "./can.h" 1
# 36 "./can.h"
extern unsigned char can_payload[5];
extern unsigned char U_ST_rx[5];
extern unsigned char P_ID_rx[5];
...
# 2 "client_receive.c" 2

void client_receive()
{
    if(client_receive_once_flag == 0)
    {
        client_receive_once_flag = 1;
        P_ID_rx[4] = '\0';
        U_ST_rx[4] = '\0';
        for (int i = 0; i < 4; i++)
        {
        ...
        if(mystrcmp(P_ID_rx, orig_PID) == 0)

        {
            client_receive_compare_flag = 1;

            U_ST = ((U_ST_rx[0] - '0') * 1000) + ((U_ST_rx[1] - '0') * 100) + ((U_ST_rx[2] - '0') * 10) + ((U_ST_rx[3] - '0') * 1);
        }
    }

    if(client_receive_compare_flag == 1)
    {
        client_receive_key = edge_dkp();
        ...
        if (client_receive_key == 0x0E) {
            U_ST++;
        }

        if (client_receive_key == 0x0D) {
            U_ST--;
        }

        if (client_receive_key == 0x0B)
        {
